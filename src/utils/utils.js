...
//const getStartDate = (items) => humanizePointDate(dayjs.min(items.map((item) => dayjs(item.dateFrom))), formatDate);

//получить самую позднюю дату из точек маршрута
//const getFinishDate = (items) => humanizePointDate(dayjs.max(items.map((item) => dayjs(item.dateTo))), formatDate);
